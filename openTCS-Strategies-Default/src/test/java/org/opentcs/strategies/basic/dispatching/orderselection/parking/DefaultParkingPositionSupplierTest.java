...
public class DefaultParkingPositionSupplierTest {
  ...
  public void setUp() {
    ...
    points = new Point[10];
    supplier = new DefaultParkingPositionSupplier(kernel, router);
  ...
  public void returnsEmptyForNoParkingPositionsFromKernel() {
    when(kernel.getTCSObjects(Point.class)).thenReturn(new HashSet<>());
    when(kernel.getTCSObjects(Block.class)).thenReturn(new HashSet<>());
    mainVehicle = new Vehicle("mainVehicle")
        .withCurrentPosition(new Point("dummyPoint").getReference());
    supplier.initialize();
    Optional<Point> result = supplier.findParkingPosition(mainVehicle);
  ...
  public void returnsEmptyForAllBlocksReserved() {
    ...
    for (int i = 0; i < points.length; i++) {
      ...
      when(kernel.getTCSObject(Point.class, points[i].getReference()))
          .thenReturn(points[i]);
    }

    when(kernel.getTCSObjects(Point.class))
  ...
  private void createBlocksAndConfigKernelBlockRequests() {
    ...
    when(kernel.getTCSObject(Block.class, blocks[0].getReference())).thenReturn(blocks[0]);
    when(kernel.getTCSObject(Block.class, blocks[1].getReference())).thenReturn(blocks[1]);
    when(kernel.getTCSObject(Block.class, blocks[2].getReference())).thenReturn(blocks[2]);
  }
}
