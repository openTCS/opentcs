...
import org.opentcs.kernel.extensions.servicewebapi.v8.binding.response.data.TransportOrderTO.DriveOrderTO.RouteTO.StepTO.ReroutingTypeTO;
...
class TransportOrderConverterTest {
  ...
  void convert() {
    ...
    TransportOrderTO result = converter.convert(order);
  ...
  void convertsAllTransportOrderStates(TransportOrder.State state) {
    ...
    TransportOrderTO.StateTO expectedState = switch (state) {
      case RAW -> TransportOrderTO.StateTO.RAW;
      case ACTIVE -> TransportOrderTO.StateTO.ACTIVE;
      case DISPATCHABLE -> TransportOrderTO.StateTO.DISPATCHABLE;
      case BEING_PROCESSED -> TransportOrderTO.StateTO.BEING_PROCESSED;
      case WITHDRAWN -> TransportOrderTO.StateTO.WITHDRAWN;
      case FINISHED -> TransportOrderTO.StateTO.FINISHED;
      case FAILED -> TransportOrderTO.StateTO.FAILED;
      case UNROUTABLE -> TransportOrderTO.StateTO.UNROUTABLE;
    };
  ...
  void convertsNullMembersToNull() {
    TransportOrder transportOrder = new TransportOrder(
        ...
        .withPeripheralReservationToken(null)
        .withIntendedVehicle(null)
        .withProcessingVehicle(null)
        .withWrappingSequence(null);

    TransportOrderTO result = converter.convert(transportOrder);

    assertThat(result.getPeripheralReservationToken()).isNull();
    assertThat(result.getIntendedVehicle()).isNull();
    assertThat(result.getProcessingVehicle()).isNull();
    assertThat(result.getWrappingSequence()).isNull();
    assertThat(result.getDriveOrders().getFirst().getRoute()).isNull();
  }

  @Test
  void convertsInstantMaxToNull() {
    TransportOrder transportOrder = new TransportOrder("order-1", List.of())
        .withDeadline(Instant.MAX)
