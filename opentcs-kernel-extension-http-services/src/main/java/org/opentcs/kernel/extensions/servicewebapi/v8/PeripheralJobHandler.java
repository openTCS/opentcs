...
public class PeripheralJobHandler {
   ...
   */
  public List<PeripheralJobTO> getPeripheralJobs(
      @Nullable
    ...
    return executorWrapper.callAndWait(() -> {
      ...
      return jobService.stream(PeripheralJob.class)
          ...
          .map(peripheralJobConverter::convert)
          .sorted(Comparator.comparing(PeripheralJobTO::getName))
   ...
   */
  public PeripheralJobTO getPeripheralJobByName(
      @Nonnull
      String name
  ) {
    requireNonNull(name, "name");
    ...
    return executorWrapper.callAndWait(() -> {
      ...
      return peripheralJobConverter.convert(job);
    });
  }
}
