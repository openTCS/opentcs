...
== Version 0.34 (2026-08-11)

* New features and enhancements:
** Do not resend a `cancelOrder` instant action once the vehicle reflects it in its `actionStates` (with any `actionStatus`).
   The driver still waits for the action to reach a final state before sending further messages.
** In Kernel Control Center, the `initPosition` action's `x`, `y`, and `theta` parameters are now sent as JSON numbers (floats) instead of strings, both in instant action and order messages.
