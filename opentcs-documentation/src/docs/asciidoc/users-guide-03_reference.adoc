...
=== Supported navigation principles / vehicle types

openTCS works independently of specific navigation implementations, so any kind of navigation principle may be used.
...
As such, the use of openTCS is not limited to a specific type of vehicle.

=== Maximum vehicle fleet size

The design of the software does not impose a specific limit on the number of vehicles that can be managed by a single openTCS instance.
The hardware equipment / runtime environment used (CPU, RAM, communication bandwidth etc.) can, however, limit the system's effective performance.

=== Requirements for vehicles

For a vehicle to be managed by openTCS, it needs to meet certain minimal set of requirements:
...
1. Communication with the vehicle must be possible.
   ...
   How this communication is implemented exactly is not really relevant as long as the communication hardware exists and there is a way for openTCS to make use of the communication channel.
   In many cases, standard Wi-Fi hardware is used.
2. The vehicle must be able to report its current position/state.
3. The vehicle must be able to perform movement orders from its current position to a nearby position in the driving course / environment given by openTCS.
...
Transport orders, order sequences, peripheral jobs and environmental entities are elements that are available only at plant operation time.
Their attributes are primarily set when the respective elements are created, and they are volatile in the sense that they are implicitly removed when a new plant model is loaded into the kernel.
...
===== Transport order priorities

With openTCS, transport orders do not have a priority attribute.
This is because a transport order's priority may change over time or when other transport orders are added.
...
Environmental entities represent objects or areas in the plant environment that are known to the system, but not controlled by it.
Typical examples are people, manually moved vehicles, temporary obstacles or dynamically restricted zones.

An environmental entity carries the following attributes:

* An _envelope_, describing the area that is occupied by the entity as a closed polygon.
  The envelope is defined with vertex coordinates relative to the plant coordinate system's origin.
* A _pose_, i.e. the entity's position in the plant coordinate system and its orientation angle.
  (The pose rotates and translates the envelope to the actual position of the entity.)
* A _type_, which is one of these:
** _Object_:
   The entity represents a tangible object in the environment.
** _Zone_:
   The entity represents an area in the environment.
* An _integration level_, indicating how far the entity is taken into account by the system.
  An entity can be
** ..._respected_:
   The entity is taken into account, e.g. for traffic management.
** ..._noticed_:
   The entity may be shown to users, but is not considered for traffic management.
** ..._ignored_:
   The entity exists, but is ignored completely.
* A _retired_ flag, indicating whether the entity is still active or has been retired.
  Retired entities are not considered for traffic management and are not shown to users.
  They are kept in the system temporarily before being removed automatically.

Environmental entities can be used for traffic management, e.g. to prevent vehicles from colliding with objects they represent or from entering zones that are currently not safe to enter.
...
NOTE: If a parking position with a higher priority is unoccupied but unreachable for a vehicle, a reachable parking position with a lower priority may be selected instead.
...
. Check if the areas related to the requested resources are available for the vehicle and don't intersect with areas allocated by other vehicles (provided that the vehicle requesting the resources references an envelope key and the requested resources define vehicle envelopes with that key).
  Area intersections are checked per plane (i.e. per Z-coordinate of points), meaning that only envelopes on the same plane are considered intersecting, and envelopes on different planes are treated as independent.
  For paths whose source and destination points are on different planes, the path envelope is considered on both planes.
...
===== Service web API v1 configuration entries

The kernel's service web API can be configured using the following configuration entries:

include::{configdoc}/ServiceWebApiV1ConfigurationEntries.adoc[]

===== Service web API v8 configuration entries

The kernel's service web API can be configured using the following configuration entries:

include::{configdoc}/ServiceWebApiV8ConfigurationEntries.adoc[]
...
===== VDA5050 configuration entries

include::{configdoc}/Vda5050ConfigurationEntries.adoc[]

===== VDA5050 MQTT configuration entries

include::{configdoc}/Vda5050MqttConfigurationEntries.adoc[]
