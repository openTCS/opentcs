...
* The order messages sent by this vehicle driver contain both the _base_ and the _horizon_ of the vehicle's route.
  How many steps of the route are sent as _base_ and _horizon_ can be configured using the vehicle properties `vda5050:maxStepsBase` and `vda5050:maxStepsHorizon` -- see <<Driver configuration>>.
