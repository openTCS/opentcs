...
* Bugs fixed:
** Avoid a `NullPointerException` in the `GET /v8/userNotifications` endpoint implementation.
* Changes affecting developers:
** Update Gradle wrapper to 9.7.0.
** Update JUnit to 6.1.3.
** Update ApprovalTests to 31.0.0.
** Update Javalin to 7.2.3.
** Update Shiro to 2.2.1.
** Update Jackson to 2.22.2.
** Update JGraphT to 1.5.3.
** Update ModelMapper to 3.2.6.

== Version 7.5.0 (2026-08-17)

* New features and enhancements:
** Integrate the latest version (0.34.0) of the VDA5050 vehicle communication adapter, which was previously distributed separately, directly into the openTCS baseline project.
   (Starting with this version, users do not need to -- and should not -- integrate the the separate VDA5050 driver into their openTCS installation anymore.)
** Let scheduler modules know when resources are being allocated forcefully (for a vehicle that has unexpectedly appeared at a position).
   Although scheduler modules cannot prevent this, the information at least gives them a chance to consider these resource allocations.

== Version 7.4.0 (2026-08-12)

* New features and enhancements:
** Introduce the concept of environmental entities, representing objects or areas in the plant model that are not under control of the kernel but which may be relevant for e.g. traffic management.
** Improve scheduler support for multi-level plants by handling vehicle envelopes per Z-plane.
** Improve performance when updating routing graphs.
** Introduce web API v8 as a preview, which is supposed to become the stable web API / replace the current web API in openTCS 8.0.
** Update web API v1 specification and implementation to version 1.15.0:
*** Extend the response body of the `GET /plantModel` endpoint and the request body of the `PUT /plantModel` endpoint to respect vehicle envelope keys.
* Bugs fixed:
** Fix integer truncation of small route cost values.
** Ensure that all routing graphs are always kept up-to-date.
** Ensure that routes for transport orders with resources to avoid are computed using up-to-date routing graphs.
** Properly consider loops that may form when vehicles are rerouted multiple times while processing the same drive order.
** When resolving coordinates to logical positions, ensure that the correct/up-to-date last known position of a vehicle is considered.
* Changes affecting developers:
** Update Gradle wrapper to 9.5.1.

== Version 7.3.0 (2026-05-26)
...
** Update a point figure in the Model Editor when its y-coordinate is changed via the properties panel.
   (Previously, only changes to the x-coordinate triggered an update.)
...
* New features and enhancements:
** Allow to be less strict with unexpected positions:
   Require manual rerouting after a vehicle driver has reported an unexpected position only if the configuration demands it.
* Changes affecting developers:
