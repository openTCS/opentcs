...
=== Creating curved paths between points

1. In the Model Editor application, select btn:[Bezier] from the path tools and connect two points by clicking on the first point, dragging the mouse to the second point and releasing the mouse button there.
2. Activate the selection tool and click on the previously created bezier path.
   Two blue control points will appear.
3. Drag the control points to change the shape of the path.

=== Automatically enabling drivers on startup

To automatically enable vehicle drivers when the kernel is started, add the following line to the kernel configuration file (`config/opentcs-kernel.properties` in the kernel application directory):
...
To enable peripheral drivers on startup, add the following line:
...
NOTE: See <<Kernel configuration>> for a description of all kernel configuration entries.
...
This section describes the included transformation.
...
The included transformation method maps coordinates from the plant model's coordinate system to the coordinate system of a vehicle (when sending coordinates to the vehicle) and vice versa (when receiving coordinates from the vehicle).
...
. In the kernel's configuration file, enable SSL for the RMI interface and/or for the web service interface.
  (See <<RMI kernel interface configuration entries>> and/or <<Service web API v1 configuration entries>> and/or <<Service web API v8 configuration entries>> for a description of the configuration entries.)
...
=== Enabling RMI on multihomed kernel host

If your kernel is running on a host with multiple IP addresses, you may observe connection problems with remote clients via RMI connections.
Clients may be unable to connect to the kernel, or communication between client and kernel may be slow, especially when SSL is enabled for the RMI interface.

In that case, set the kernel JVM's property `java.rmi.server.hostname` to the correct IP address of the machine running it.
(Also see https://docs.oracle.com/javase/8/docs/technotes/guides/rmi/faq.html#netmultihomed.)
...
=== Integrating a vehicle that supports VDA5050

openTCS comes with a VDA5050 driver that can be used to integrate vehicles supporting the VDA5050 protocol.
To do this, you need to configure the driver and the vehicle in the plant model accordingly:

. In the kernel configuration, ensure that the VDA5050 version you intend to use is enabled, and that the connection details for your MQTT broker are correct.
  (See <<VDA5050 configuration entries>> and <<VDA5050 MQTT configuration entries>> for details.)
. In the plant model, create a vehicle and set the required properties for the VDA5050 driver to be able to connect to the vehicle.
  (Depending on which version of VDA5050 you want to work with, see link:./vda5050-1.1.html[the VDA5050 1.1 Integration Guide] or link:./vda5050-2.0.html[the VDA5050 2.0 Integration Guide] for details.)
. Start the kernel with the plant model and enable the VDA5050 driver via the Kernel Control Center.
  The driver will then connect to the MQTT broker.
  Assuming the vehicle publishes state updates and both the VDA5050 driver and the vehicle are configured for the same topics, openTCS will receive and process these state updates.
  In case the vehicle is in a state that allows it to accept orders, you can create a suitable transport order and have it assigned to the vehicle, which should then process it.
