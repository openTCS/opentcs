 ...
 */
public class StandardVehicleController
   ...
   */
  @Inject
  public StandardVehicleController(@Assisted Vehicle vehicle,
                                   @Assisted CommunicationAdapter adapter,
                                   LocalKernel kernel,
                                   Scheduler scheduler,
                                   MBassador<Object> eventBus) {
    controlledVehicle = requireNonNull(vehicle, "vehicle");
  ...
  public void abortDriveOrder() {
    ...
    synchronized (commAdapter) {
      if (currentDriveOrder == null) {
        log.fine(vehicleName + ": No drive order to be aborted");
        return;
      }
  ...
  public boolean allocationSuccessful(final Set<TCSResource> resources) {
    ...
    synchronized (commAdapter) {
      ...
      checkState(commAdapter.addCommand(command),
  ...
  @Override
  public void publishEvent(CommunicationAdapterEvent event) {
  ...
  public void enable() {
    ...
    if (!enabled) {
      ...
      log.fine("Already enabled, doing nothing.");
    }
  }
  ...
  private void updatePositionWithOrder(String position, Point point) {
    ...
    updatePosition(toReference(point),
                   extractNextPosition(findNextCommand()));
  }
