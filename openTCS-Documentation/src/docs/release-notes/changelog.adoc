...
** Add support for specifying and retrieving the following properties of transport orders via web API: dispensability, peripheral reservation token, wrapping sequence, type.
** Add two new ways to trigger the dispatcher via web API: `POST /vehicles/dispatcher/trigger` and `POST /transportOrders/dispatcher/trigger`.
   The old `POST /dispatcher/trigger` is now deprecated.
* Bugs fixed:
** Fix a bug where (immediately) withrawing a transport order from a vehicle that is waiting for a peripheral job to finish would prevent any further commands (e.g. for new transport orders) to be sent to the vehicle.
