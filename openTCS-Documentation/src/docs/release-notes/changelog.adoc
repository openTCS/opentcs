...
*** Migration note: Integration projects need to update any use of slf4j providers to the same version, too, or the respective logging backend might not be used.
** Update Guice to 7.0.0.
*** Migration note: Integration projects now need to use injection-related annotations in the `jakarta.inject` namespace, e.g. `jakarta.inject.Inject` or `jakarta.inject.Provider`.
