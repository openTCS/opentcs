...
Locations and vehicles are visualized in the plant overview client using pluggable themes.
...
. Create a Guice module for your theme by creating a class inheriting `PlantOverviewInjectionModule`.
Implement the `configure()` method and add a binding to your theme using `vehicleThemeBinder()` or `locationThemeBinder()`.
The following example demonstrates how this module looks like for the themes that are part of the openTCS distribution:
+
[source, java]
----
include::{plantoverview-guiceSrc}/org/opentcs/guing/DefaultThemesModule.java[tags=documentation_createThemeModule]
----

. Ensure that the JAR file containing your theme(s) also contains a folder named `META-INF/services/` with a file named `org.opentcs.customizations.plantoverview.PlantOverviewInjectionModule`.
This file should consist of a single line of text holding simply the name of the implementing class, e.g.:
+
[source]
----
org.opentcs.guing.DefaultThemesModule
----
+
...
Themes meeting these requirements are found automatically when you start the plant overview client.
