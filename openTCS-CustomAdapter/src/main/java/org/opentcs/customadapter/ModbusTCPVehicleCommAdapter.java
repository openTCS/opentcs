...
//        getProcessModel().setLength(configuration.vehicleLengthUnloaded());
  ...
  private void initializeMCPool(MovementCommand cmd) {
    ...
    List<MovementCommand> movementCommands = getAllMovementCommands();
...
//    }
  ...
  /**
   * Retrieves all movement commands from the ModbusTCPVehicleCommAdapter.
   *
   * <p>This method returns a list containing all unsent and sent movement commands
   * stored in the ModbusTCPVehicleCommAdapter. The list is synchronized to ensure thread
   * safety.</p>
   *
   * @return A list of MovementCommand objects representing all movement commands in the
   * ModbusTCPVehicleCommAdapter.
   */
  public List<MovementCommand> getAllMovementCommands() {
    List<MovementCommand> allCommands = new ArrayList<>();
    synchronized (this) {
      allCommands.addAll(getSentCommands());
      allCommands.addAll(getUnsentCommands());
    }
    LOG.info(String.format("The length of all MovementCommands is: %d", allCommands.size()));
    return allCommands;
  }
