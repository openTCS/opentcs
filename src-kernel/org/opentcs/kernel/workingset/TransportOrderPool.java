...
public class TransportOrderPool {
  ...
  private static final Logger log
      = Logger.getLogger(TransportOrderPool.class.getName());
  /**
  ...
  public TransportOrder createTransportOrder(
    ...
    TransportOrder newOrder
        = new TransportOrder(orderID, orderName, destinations);
  ...
  public boolean hasUnfinishedDependencies(
      TCSObjectReference<TransportOrder> orderRef) {
    log.finer("method entry");
    Objects.requireNonNull(orderRef, "orderRef is null");
    ...
    // Check explicit dependencies of the transport order.
    for (TCSObjectReference<TransportOrder> curReference
         : order.getDependencies()) {
      TransportOrder curDep = objectPool.getObject(TransportOrder.class,
                                                   curReference);
      // The referenced transport order either doesn't exist any more or is not
      // finished.
      if (curDep != null && !curDep.hasState(TransportOrder.State.FINISHED)) {
        return true;
      }
    }
    ...
    if (order.getWrappingSequence() != null) {
      OrderSequence seq = getOrderSequence(order.getWrappingSequence());
      if (!orderRef.equals(seq.getNextUnfinishedOrder())) {
        return true;
      }
    }
