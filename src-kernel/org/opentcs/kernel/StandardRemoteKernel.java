...
class StandardRemoteKernel
    implements KernelExtension, EventListener<TCSEvent>, InvocationHandler {
  ...
  public void plugIn() {
    ...
    if (Objects.equals(registryAddress.getHost(), "localhost")) {
      registry = RMIRegistries.getOrCreateWorkingRegistry(registryAddress.getPort());
    }
    else {
      registry = RMIRegistries.getWorkingRegistry(registryAddress.getHost(),
                                                  registryAddress.getPort());
    }
    if (registry == null) {
      // No registry available - abort startup.
      throw new IllegalStateException("RMI registry unavailable");
    }
