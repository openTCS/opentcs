...
final class VelocityController {
   ...
   */
  private static final String CLASSNAME = VelocityController.class.getName();
  /**
   * This class's Logger.
   */
   ...
   */
  private final Set<VelocityListener> velocityListeners =
      new HashSet<>();
  /**
    ...
    if (curWayEntry == null || paused) {
      ...
      switch (orientation) {
        ...
        case BACKWARD:
          ...
          break;
        default:
          log.warning("Unhandled orientation: " + orientation.toString()
              + ", assuming forward");
          maxVelocity = maxFwdVelocity;
      }
      ...
      if ((curWayEntry != null) && (currentPosition >= curWayEntry.length)) {
  ...
  static class WayEntry
     ...
     */
    public WayEntry(long length, int maxVelocity, String destPointName,
      ...
      this.length = length;
      if (maxVelocity < 1) {
        log.severe("maxVelocity is zero or negative, setting to 100 as a workaround");
        this.targetVelocity = 100;
      }
      else {
        this.targetVelocity = maxVelocity;
      }
      this.destPointName = Objects.requireNonNull(destPointName,
                                                  "destPointName is null");
      this.vehicleOrientation = Objects.requireNonNull(orientation,
                                                       "orientation is null");
    }
