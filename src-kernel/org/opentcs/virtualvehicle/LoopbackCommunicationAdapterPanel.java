...
import org.opentcs.drivers.CommunicationAdapterView;
 ...
 */
final class LoopbackCommunicationAdapterPanel
    extends JPanel
    implements CommunicationAdapterView {

  /**
   * This class's Logger.
   */
  private static final Logger log
      = Logger.getLogger(LoopbackCommunicationAdapterPanel.class.getName());
  /**
   ...
   */
  LoopbackCommunicationAdapterPanel(LoopbackCommunicationAdapter adapter) {
  ...
  private void updateGui() {
    setStatePanelEnabled(commAdapter.isEnabled());
    chkBoxEnable.setSelected(commAdapter.isEnabled());
    pauseVehicleCheckBox.setSelected(commAdapter.isVehiclePaused());
    boolean stepModeEnabled = commAdapter.isSingleStepModeEnabled();
    ...
    maxFwdVeloTxt.setText(Integer.toString(commAdapter.getMaxFwdVelocity()));
    maxRevVeloTxt.setText(Integer.toString(commAdapter.getMaxRevVelocity()));
    maxAccelTxt.setText(Integer.toString(commAdapter.getMaxAcceleration()));
    maxDecelTxt.setText(Integer.toString(commAdapter.getMaxDeceleration()));
    defaultOpTimeTxt.setText(
        Integer.toString(commAdapter.getDefaultOperatingTime()));
    // Update current vehicle position
    String currentPos = commAdapter.getVehiclePosition();
    if (currentPos == null) {
      positionTxt.setText("");
    }
    else {
      for (Point curPoint : commAdapter.getModelPoints()) {
        if (curPoint.getName().equals(commAdapter.getVehiclePosition())) {
    ...
    stateTxt.setText(commAdapter.getVehicleState().toString());
    // Update load handling devices
    LoadHandlingDeviceTableModel model
        = (LoadHandlingDeviceTableModel) devicesTable.getModel();
    model.updateLoadHandlingDevices(commAdapter.getVehicleLoadHandlingDevices());
    // Update precise position fields
    Triple curPos = commAdapter.getVehiclePrecisePosition();
    if (curPos == null) {
    ...
    double curOrientation = commAdapter.getVehicleOrientationAngle();
    ...
    movementPowerTxt.setText(Double.toString(commAdapter.getMovementPower()));
    operationPowerTxt.setText(Double.toString(commAdapter.getOperationPower()));
    idlePowerTxt.setText(Double.toString(commAdapter.getIdlePower()));
  ...
  private void applyOperationSpecs() {
    ...
    commAdapter.setOperationSpecs(opSpecsMap);
  }
  ...
  private void initComponents() {
    ...
    operationSpecList = new javax.swing.JList<OperationSpec>();
    ...
    opSpecDeviceList = new javax.swing.JList<String>();
  ...
  private void singleModeRadioButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_singleModeRadioButtonActionPerformed
    if (singleModeRadioButton.isSelected()) {
      ...
      commAdapter.setSingleStepModeEnabled(true);
    }
  ...
  private void flowModeRadioButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_flowModeRadioButtonActionPerformed
    if (flowModeRadioButton.isSelected()) {
      triggerButton.setEnabled(false);
      commAdapter.setSingleStepModeEnabled(false);
      commAdapter.trigger();
    }
...
private void rmOpSpecButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_rmOpSpecButtonActionPerformed
  ...
  private void loadPropertiesFromProfile(VehicleProfile profile) {
    ...
    commAdapter.setIdlePower(profile.getIdlePower());
    commAdapter.setMovementPower(profile.getMovementPower());
    commAdapter.setOperationPower(profile.getOperationPower());
    commAdapter.setMaxFwdVelocity(profile.getFwdVelocity());
    commAdapter.setMaxRevVelocity(profile.getRevVelocity());
    commAdapter.setMaxAcceleration(profile.getAcceleration());
    commAdapter.setMaxDeceleration(profile.getDeceleration());
    commAdapter.setDefaultOperatingTime(profile.getDefaultOpTime());
    ...
    commAdapter.setOperationSpecs(opSpecs);
  ...
  private VehicleProfile storePropertiesInProfile(String name) {
    ...
    profile.setIdlePower(commAdapter.getIdlePower());
    profile.setMovementPower(commAdapter.getMovementPower());
    profile.setOperationPower(commAdapter.getOperationPower());
    profile.setFwdVelocity(commAdapter.getMaxFwdVelocity());
    profile.setRevVelocity(commAdapter.getMaxRevVelocity());
    profile.setAcceleration(commAdapter.getMaxAcceleration());
    profile.setDeceleration(commAdapter.getMaxDeceleration());
    profile.setDefaultOpTime(commAdapter.getDefaultOperatingTime());
    // Store a copy of the OperationSpec-list
    List<OperationSpec> opSpecList = new LinkedList<>(
        commAdapter.getOperationSpecs().values());
  ...
  private void maxFwdVeloTxtMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_maxFwdVeloTxtMouseClicked
    ...
    if (dialog.getReturnStatus() == InputDialog.ReturnStatus.ACCEPTED) {
      ...
      commAdapter.setMaxFwdVelocity(velocity);
    }
    update();
  ...
  private void maxRevVeloTxtMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_maxRevVeloTxtMouseClicked
    ...
    if (dialog.getReturnStatus() == InputDialog.ReturnStatus.ACCEPTED) {
      int velocity = Integer.parseInt((String) dialog.getInput());
      commAdapter.setMaxRevVelocity(velocity);
    }
    update();
  ...
  private void maxAccelTxtMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_maxAccelTxtMouseClicked
    ...
    if (dialog.getReturnStatus() == InputDialog.ReturnStatus.ACCEPTED) {
      ...
      commAdapter.setMaxAcceleration(acceleration);
    }
    update();
  ...
  private void maxDecelTxtMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_maxDecelTxtMouseClicked
    ...
    if (dialog.getReturnStatus() == InputDialog.ReturnStatus.ACCEPTED) {
      ...
      commAdapter.setMaxDeceleration(deceleration);
    }
    update();
  ...
  private void defaultOpTimeTxtMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_defaultOpTimeTxtMouseClicked
    ...
    if (dialog.getReturnStatus() == InputDialog.ReturnStatus.ACCEPTED) {
      ...
      commAdapter.setDefaultOperatingTime(opTime);
    }
    update();
  ...
  private void movementPowerTxtMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_movementPowerTxtMouseClicked
    if(movementPowerTxt.isEnabled()) {
      ...
      if (dialog.getReturnStatus() == InputDialog.ReturnStatus.ACCEPTED) {
        ...
        commAdapter.setMovementPower(power);
      }
      update();
    }
  ...
  private void operationPowerTxtMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_operationPowerTxtMouseClicked
    if(operationPowerTxt.isEnabled()) {
      ...
      if (dialog.getReturnStatus() == InputDialog.ReturnStatus.ACCEPTED) {
        double power = Double.parseDouble((String) dialog.getInput());
        commAdapter.setOperationPower(power);
      }
      update();
    }
  ...
  private void idlePowerTxtMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_idlePowerTxtMouseClicked
    if(idlePowerTxt.isEnabled()) {
      ...
      if (dialog.getReturnStatus() == InputDialog.ReturnStatus.ACCEPTED) {
        double power = Double.parseDouble((String) dialog.getInput());
        commAdapter.setIdlePower(power);
      }
      update();
    }
  ...
  private void precisePosTextAreaMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_precisePosTextAreaMouseClicked
    if (precisePosTextArea.isEnabled()) {
      Triple pos = commAdapter.getVehiclePrecisePosition();
      ...
      if (dialog.getReturnStatus() == InputDialog.ReturnStatus.ACCEPTED) {
        if (dialog.getInput() == null) {
          ...
          commAdapter.setVehiclePrecisePosition(null);
        }
        else {
          ...
          commAdapter.setVehiclePrecisePosition(new Triple(x, y, z));
        }
        update();
      }
    }
  ...
  private void stateTxtMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_stateTxtMouseClicked
    if (stateTxt.isEnabled()) {
      ...
      Vehicle.State currentState = commAdapter.getVehicleState();
      // Create panel and dialog
      InputPanel panel = new DropdownListInputPanel.Builder<>(
      ...
      if (dialog.getReturnStatus() == InputDialog.ReturnStatus.ACCEPTED) {
        ...
        if (newState != currentState) {
          commAdapter.setVehicleState(newState);
          update();
        }
      }
    }
  ...
  private void positionTxtMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_positionTxtMouseClicked
    if (positionTxt.isEnabled()) {
      ...
      Set<Point> pointSet = commAdapter.getModelPoints();
      List<Point> pointList = new ArrayList<>(pointSet);
      Collections.sort(pointList, TCSObject.nameComparator);
      pointList.add(0, null);
      // Get currently selected point
      // TODO is there a better way to do this?
      Point currentPoint = null;
      String currentPointName = commAdapter.getVehiclePosition();
      ...
      if (dialog.getReturnStatus() == InputDialog.ReturnStatus.ACCEPTED) {
        ...
        if (item == null) {
          commAdapter.setVehiclePosition(null);
        }
        else {
          commAdapter.setVehiclePosition(item.toString());
        }
        update();
      }
    }
  ...
  private void orientationAngleTxtMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_orientationAngleTxtMouseClicked
    if (orientationAngleTxt.isEnabled()) {
      double currentAngle = commAdapter.getVehicleOrientationAngle();
      String initialValue
      ...
      if (returnStatus == InputDialog.ReturnStatus.ACCEPTED) {
        ...
        if (input == null) { // The reset button was pressed
          if (!Double.isNaN(commAdapter.getVehicleOrientationAngle())) {
            commAdapter.setVehicleOrientationAngle(Double.NaN);
          }
        }
        else {
          ...
          commAdapter.setVehicleOrientationAngle(angle);
        }
      }
      update();
    }
  ...
  private void pauseVehicleCheckBoxItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_pauseVehicleCheckBoxItemStateChanged
    if (evt.getStateChange() == java.awt.event.ItemEvent.SELECTED) {
      commAdapter.setVehiclePaused(true);
    }
    else if (evt.getStateChange() == java.awt.event.ItemEvent.DESELECTED) {
      commAdapter.setVehiclePaused(false);
    }
  ...
  private void saveDeviceButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_saveDeviceButtonActionPerformed
    ...
    commAdapter.setVehicleLoadHandlingDevices(model.getLoadHandlingDevices());
  ...
  private class OpSpecListListener
    ...
    private void update() {
      OperationSpec selection
          = (OperationSpec) operationSpecList.getSelectedValue();
