...
public class TCSObjectPoolTest {
  ...
  public void testEventListener() {
    EventBuffer<TCSEvent> eventBuffer = new EventBuffer<>(new EventFilter<TCSEvent>() {
    ...
    pool = new TCSObjectPool(eventBuffer);
    Point point1 = new Point(pool.getUniqueObjectId(), "Point-00001");
    pool.addObject(point1);

    assertEquals(0, eventBuffer.getEvents(0).size());
    pool.emitObjectEvent(point1, point1, TCSObjectEvent.Type.OBJECT_CREATED);
    assertEquals(1, eventBuffer.getEvents(0).size());
  }
